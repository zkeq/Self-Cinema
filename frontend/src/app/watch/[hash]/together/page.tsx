...
export default function TogetherPage() {
  ...
  const shareLink = useMemo(() => {
    ...
    url.pathname = `/watch/${hash}/together`;
    ...
    window.postMessage(
      {
        type: 15,
        source: "VideoTogether",
        data: { key: "DisableRedirectJoin", value: true },
      },
      "*",
    );
  }, []);
  ...
  }, [applyDefaultSettings, roomName, roomPassword]);
  ...
  const connectChat = useCallback(() => {
    const syncMessages = async () => {
      try {
        ...
        if (lastTimestampRef.current) {
          qs.set("since", lastTimestampRef.current);
        }
        const res = await fetch(`${apiBaseUrl}/together/${hash}/messages?${qs.toString()}`, {
          cache: "no-store",
        });
        if (!res.ok) {
        ...
        console.error("poll chat failed", err);
        setChatStatus("error");
      }
    };
    ...
    syncMessages();
    pollingRef.current = setInterval(syncMessages, 2500);
  }, [apiBaseUrl, hash]);

  const postChatMessage = useCallback(
    async (payload: ChatMessage) => {
      try {
        ...
        if (Array.isArray(data)) {
          setMessages((prev) => {
            const merged = new Map<string, ChatMessage>();
            [...prev, ...data].forEach((msg) => merged.set(msg.id, msg));
            return Array.from(merged.values()).sort(
              (a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime(),
            );
          });
  ...
  useEffect(() => {
    setIsHost(roomAction === "create");
  }, [roomAction]);

  useEffect(() => {
    if (chatStatus === "connected" && !joinAnnouncedRef.current) {
  ...
  }, [chatStatus, displayName, generateId, postChatMessage]);
  ...
  }, [applyDefaultSettings, vtScriptLoaded]);
              ...
              <CardContent className="space-y-4">
                <div className="grid md:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="text-sm rounded-md border border-dashed border-border/70 bg-muted/50 px-3 py-2 text-muted-foreground">
                      已启用密码保护（自动附在分享链接中，不在页面展示）
                    </div>
                  </div>
                </div>
                <div className="space-y-1">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription className="space-y-2 text-sm">
                  ...
                  <p>2. 默认开启“窗口默认最小化”和“加入房间时不要自动跳转”。</p>
                  <p>3. 将上方分享链接发给好友（自带房间密码），访客默认执行 Join 进入房间。</p>
